...
```bash
python3 scripts/generate_goal.py --questions-json "给项目加单元测试"
```

该功能适合 IDE、表单、机器人或工作流系统读取后自动渲染补信息界面。
...
使用 `--redaction-check` 可在把任务描述分享到机器人、Issue、PR 或外部系统前，检查是否包含疑似 token、密钥、邮箱、URL 等敏感片段，并输出脱敏预览和处理建议：
...
# 输出按可执行度等级分组的 Markdown 评审看板，适合团队批量需求评审
python3 scripts/batch_generate.py --input examples/sample_tasks.json --review-board-md review_board.md

# 输出批量风险报告，适合按风险分数审计任务清单
python3 scripts/batch_generate.py --input examples/sample_tasks.json --risk-report-md risk_report.md

# 输出每个任务可直接发送给需求方的追问 Markdown 包，适合批量补信息
python3 scripts/batch_generate.py --input examples/sample_tasks.json --questions-md questions.md

# 输出批量敏感信息报告，适合任务清单分发前做脱敏审计
python3 scripts/batch_generate.py --input examples/sample_tasks.json --redaction-report-md redaction_report.md

# 输出批量就绪度矩阵，横向查看每个任务 6 要素状态、评分、风险和下一步动作
python3 scripts/batch_generate.py --input examples/sample_tasks.json --readiness-matrix-md readiness_matrix.md

# 输出单个 JSONL 草稿文件，适合流水线或机器人逐行消费每个任务的 6 要素和复核状态
python3 scripts/batch_generate.py --input examples/sample_tasks.json --draft-jsonl drafts.jsonl
