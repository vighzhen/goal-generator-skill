...
# 主流程任务风险阈值门禁：生成或 dry-run 前直接阻断 medium/high 风险任务
python3 scripts/batch_generate.py --input examples/sample_tasks.json --dry-run --fail-on-task-risk-level medium --report-json risk_gate_report.json
...
- `--fail-on-task-risk-level <low|medium|high>` 可用于真实生成、`--dry-run`、`--check` 或 `--lint-output` 前的准备流程；它复用任务画像风险识别，把达到指定等级及以上的任务跳过并返回非零退出码，适合不想把画像预检拆成单独命令的 CI 或批量生成脚本。若需要完整画像分布和风险因素详情，仍建议先运行 `--profile-tasks`。
